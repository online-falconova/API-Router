...
export function SkillsConceptCard({
  ...
  const crossLinkHref =
    variant === "agent" ? "/dashboard/api-router-skills" : "/dashboard/agent-skills";
  ...
  return (
    ...
    >
      ...
      {/* Comparison table */}
      <div className="rounded-lg border border-black/5 dark:border-white/5 overflow-x-auto overscroll-x-contain">
        {/* Column headers */}
        <div className="grid min-w-[30rem] grid-cols-3 bg-bg-subtle border-b border-black/5 dark:border-white/5">
        ...
        {COMPARISON_ROWS.map((row: ComparisonRow, i) => (
          <div
            ...
            className={`grid min-w-[30rem] grid-cols-3 ${i < COMPARISON_ROWS.length - 1 ? "border-b border-black/5 dark:border-white/5" : ""}`}
          >
