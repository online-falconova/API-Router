 ...
 * `thinkingConfig`, …). The API Router router endpoint `/v1/chat/completions`
 ...
 *   - forward to `/v1/chat/completions` on the API Router router,
 ...
 * @param model      resolved API Router model string
  ...
  async intercept(
    ...
    try {
      ...
      if (!upstream.ok) {
        ...
        throw new Error(`API Router ${upstream.status}: ${errText}`);
      }
