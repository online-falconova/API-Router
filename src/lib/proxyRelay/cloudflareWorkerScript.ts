/**
 * Cloudflare Worker source emitter for the API Router proxy relay.
 *
 ...
 *
 * The API Router variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerScript(relayAuth: string): string {
  ...
  return `// API Router Cloudflare Worker proxy relay — generated at deploy time.
