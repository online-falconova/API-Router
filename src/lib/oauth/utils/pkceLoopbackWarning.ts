 ...
 * browser, not on whatever host serves the API Router dashboard.
 *
 ...
 *   API Router *server's* loopback while the provider redirects the *browser's* loopback.
 */
...
export function buildPkceLoopbackMismatchWarning(provider: string): string {
  ...
  return (
    `API Router is being accessed from a LAN IP, not true localhost. ${provider}'s OAuth app ` +
    `is registered with a fixed loopback redirect (${redirect}) that only resolves on the ` +
    "machine running this browser tab, not on the API Router server — the login will silently " +
    "fail on the provider's side. Open the API Router dashboard from true localhost instead " +
