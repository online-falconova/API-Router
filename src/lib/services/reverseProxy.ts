 ...
 * can be iframed inside the API Router dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between API Router and the embedded service.
 ...
 * Prevents API Router session cookies and Authorization from leaking upstream.
 */
 ...
 *   API Router origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the API Router iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (API Router controls framing via its own CSP).
