 ...
 *      API Router itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      API Router's own `/v1` so the vision model can be any provider
 *      registered in API Router (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. API Router self-loop (`http://localhost:20128/v1`) — auto-detected when
 ...
 *                that require API Router self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through API Router's own router,
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // API Router itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the API Router self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so API Router can resolve the
      ...
      // When routing through API Router's own API, omit the vision-bridge
