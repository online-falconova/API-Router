# API Router A2A Server

> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use API Router as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes API Router as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through API Router intelligent pipeline",
...
Routes prompts through API Router's intelligent pipeline with full observability.
...
Discovers API Router agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use API Router A2A as a custom LLM.
"""
...
An orchestrator agent delegates code generation to API Router, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via API Router A2A
    ...
    # Step 2: Review the code via API Router A2A (different model)
...
Multiple agents share quota through API Router, using the quota skill to coordinate.
...
Part of [API Router](https://github.com/cloud-falconova/API-Router) — MIT License © Falconova.com.
