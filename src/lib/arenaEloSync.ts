...
/**
 * Maps Arena leaderboard categories to API Router task-type categories.
 *
...
/**
 * API Router model aliases: canonical name → known aliases.
 * Creates additional DB entries for each alias so that models
 * are findable under any name API Router uses internally.
 */
 ...
 *
 * Leaderboard categories are mapped to API Router task types:
 * - "text" → default, review, documentation, debugging
 * - "code" → coding
 *
 * Known API Router model aliases are also expanded into additional entries.
 *
